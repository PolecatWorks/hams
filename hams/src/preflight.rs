...
impl Checks {
    pub async fn preflight(&self, client: &Client) -> Result<u32, HamsError> {
        ...
        if fails > 0 {
            ...
            Err(HamsError::PreflightCheck)
        }
    }
    pub async fn shutdown(&self, client: &Client) -> Result<u32, HamsError> {
        let mut fails = self.fails;
