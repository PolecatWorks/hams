...
impl Hams {
    ...
    fn add_alive(&self, newval: Box<dyn HealthCheck>) {
        self.alive
            .lock()
            .unwrap()
            .insert(HealthCheckWrapper(newval));
    ...
    fn remove_alive(&mut self, alive: Box<dyn HealthCheck>) -> bool {
...
mod warp_filters {
    ...
    pub fn hams_service(
        ...
        warp::path("health").and(name.or(version).or(alive).or(ready))
    }
